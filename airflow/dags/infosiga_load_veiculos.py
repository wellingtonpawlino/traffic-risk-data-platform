...
def load_veiculos(**context):
    s3 = _s3_client()
    dfs = []
    tmp_dir = tempfile.mkdtemp()

    for dataset in VEICULOS_DATASETS:
    ...
    print(f"🔥 Total veículos: {df_final.shape}")
    ...
    df_final.head(0).to_sql("veiculos", engine, schema="prep", if_exists="append", index=False)

    with engine.begin() as conn:
        conn.execute(text("TRUNCATE TABLE prep.veiculos;"))

    df_final.to_sql("veiculos", engine, schema="prep", if_exists="append", index=False, chunksize=50000)

    print("✅ prep.veiculos carregado")


with DAG(
    dag_id="infosiga_load_veiculos",
    ...
    PythonOperator(
        task_id="load_veiculos",
        python_callable=load_veiculos
    )
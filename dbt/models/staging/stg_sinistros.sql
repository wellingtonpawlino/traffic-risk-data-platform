SELECT
    ...
    ,CASE
        WHEN tipo_registro IS NULL  
             OR TRIM(tipo_registro) = '' 
             OR LOWER(TRIM(tipo_registro)) = 'nao disponivel'
   ...
   ,CASE
        WHEN logradouro IS NULL 
             OR TRIM(logradouro) = '' 
             OR LOWER(TRIM(logradouro)) = 'nao disponivel'
        THEN 'Sem Informacoes'
   ...
   ,CASE
        WHEN tp_sinistro_primario IS NULL 
             OR TRIM(tp_sinistro_primario) = '' 
             OR LOWER(TRIM(tp_sinistro_primario)) = 'nao disponivel'
        THEN 'Sem Informacoes'
